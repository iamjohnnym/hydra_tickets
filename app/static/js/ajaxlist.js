            ...
            if (data['num_results'] > self.cb) {
                self.ajax(self.callbacksURI, 'GET').done(function(data) {
                    ...
                    for (var i = 0; i < data.objects.length; i++) {
                        if (data.objects[i]['details'][data.objects[i]['details'].length - 1].status != 'closed') {
                            ...
                            self.callbacks.push({
                                ...
                                platform: ko.observable(data.objects[i].platform.capitalize()),
                                platform_fa: ko.observable(platform_fa),
                                uri: ko.observable('/callbacks/'+data.objects[i].id),
                                ticket_url: ko.observable('https://to.remove/tickets/'+data.objects[i].ticket),
                                ddi_url: ko.observable('https://customer.account/customer/'+data.objects[i].ddi),
                                status: ko.observable(data.objects[i]["details"][data.objects[i]["details"].length - 1].status.capitalize()),
